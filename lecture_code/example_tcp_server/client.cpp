...
void send_message(const Message& msg) {
    ...
    if (client_socket != -1) {
        ssize_t bytes_sent = send(client_socket, &msg, sizeof(Message), 0);
        if (bytes_sent == -1) {
...
Message receive_message() {
    ...
    if (client_socket != -1) {
        ...
        if (bytes_received <= 0) {
            ...
            print_debug("Received message: type=" + to_string(msg.type) + ", client_id=" + to_string(msg.client_id) + ", content=" + string(msg.content));
        }
    } else {
        msg.client_id = -1;
    }
...
void* receive_thread(void* /*arg*/) {
    ...
    while (client_running) {
        ...
        if (msg.client_id == -1) {
            ...
            break;
        } else if (msg.client_id == 0 && strlen(msg.content) == 0) {
...
void* input_thread(void* /*arg*/) {
    ...
    while (client_running) {
        ...
        if (input.substr(0, 5) == "/say ") {
            ...
            strncpy(msg.content, message_text.c_str(), sizeof(msg.content) - 1);
